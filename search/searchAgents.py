...
def cornersHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    maxDis = ((0, 0), (0, 0), 0)
    node = [item for item in corners if item not in state[1]]
    if len(node) == 0:
        return 0
    for x in node:
        for y in node:
            if x != y:
                dis = mazeDistance(x, y, problem.startingGameState)
                if maxDis[2] < dis:
                    maxDis = (x, y, dis)
    if maxDis[0] == (0, 0) and maxDis[1] == (0, 0):
        cost = mazeDistance(state[0], node[0], problem.startingGameState)
    else:
        d1 = mazeDistance(state[0], maxDis[0], problem.startingGameState)
        d2 = mazeDistance(state[0], maxDis[1], problem.startingGameState)
        cost = min(d1, d2) + maxDis[2]
    return cost
...
#                     maxDis = (x, y, dis)
#     if maxDis[0] == (0, 0) and maxDis[1] == (0, 0):
#         cost = mazeDistance(position, foods[0], problem.startingGameState)
#     else:
#         d1 = mazeDistance(maxDis[0], position, problem.startingGameState)
#         d2 = mazeDistance(maxDis[1], position, problem.startingGameState)
#         cost = maxDis[2] + min(d1, d2)
#     return cost

#     node = [item for item in corners if item not in state[1]]
#     return getStraight(node, state[0])
#
#
# def getStraight(corners, pos):
#     """ 假设没有墙，寻找遍历所有角落的最短距离 """
#     if len(corners) == 0:
#         return 0
#     ans = 999999
#     for cur in corners:
#         dis = util.manhattanDistance(cur, pos) + getStraight(
#             [remain for remain in corners if remain != cur], cur)
#         ans = min(dis, ans)
#     return ans
    ...
    def findPathToClosestDot(self, gameState):
        ...
        for x in range(food.width):
            for y in range(food.height):
                if food.data[x][y]:
                    dis = util.manhattanDistance((x, y), startPosition)
                    if dis < minDis:
                        minDis = dis
